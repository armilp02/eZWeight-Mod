...
public class WeightListWidget extends ObjectSelectionList<WeightListWidget.Entry> {
    ...
    public static class CategoryEntry extends Entry {
        ...
        public CategoryEntry(String namespace) {
            this.title = new TextComponent("§6[" + namespace + "]");
        }

        @Override
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height,
                           int mouseX, int mouseY, boolean hovered, float partialTick) {
            Minecraft.getInstance().font.draw(poseStack, title, left + 5, top + 5, 0xFFFFAA);
        }
    ...
    public static class SeparatorEntry extends Entry {
        ...
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height,
                           int mouseX, int mouseY, boolean hovered, float partialTick) {
            fill(poseStack, left + 20, top + height / 2, left + width - 20, top + height / 2 + 1, 0x40FFFFFF);
        }
    ...
    public class ItemEntry extends Entry {
        ...
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height,
            ...
            if (selectedEntries.contains(this)) {
                fill(poseStack, left, top, left + width, top + height, 0x8000FF00);
            } else if (hovered) {
                fill(poseStack, left, top, left + width, top + height, 0x80808080);
            }

            if (hasGunId) {
                fill(poseStack, left + 2, top + 2, left + 6, top + 6, 0xFF00AAFF);
            }

            Minecraft mc = Minecraft.getInstance();
            mc.getItemRenderer().renderGuiItem(stack, left + 5, top + 2);
            mc.getItemRenderer().renderGuiItemDecorations(mc.font, stack, left + 5, top + 2);
            ...
            String weightText = new TranslatableComponent("tooltip.ezweight.item_weight", String.format("%.2f", totalWeight)).getString();
            ...
            mc.font.draw(poseStack, itemName + idText, left + 26, top + 4, color);
            mc.font.draw(poseStack, weightText, left + 26, top + 14, 0xAAAAAA);
        }
        ...
        public Component getNarration() {
            ...
            return new TextComponent(stack.getHoverName().getString() + " - Weight: " + weight + " - ID: " + effectiveId + gunIdInfo);
        }
    }
}