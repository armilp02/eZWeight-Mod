...
public class ItemTooltipHandler {
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (!stack.isEmpty()) {
            ...
            double totalWeight = PlayerWeightHandler.getExtendedStackWeightWithContents(stack, player);
            ...
            if (totalWeight > 0.0) {
                ...
                if (totalWeight == baseWeight * stack.getCount()) {
                    event.getToolTip().add(new TranslatableComponent("tooltip.ezweight.item_weight", String.format("%.2f", totalWeight))
                            .setStyle(Style.EMPTY.withColor(baseColorRgb)));
                } else {
                    if (baseWeight > 0.0) {
                        event.getToolTip().add(new TranslatableComponent("tooltip.ezweight.item_weight_base", String.format("%.2f", baseWeight))
                                .setStyle(Style.EMPTY.withColor(baseColorRgb)));
                    }

                    event.getToolTip().add(new TranslatableComponent("tooltip.ezweight.item_weight_total", String.format("%.2f", totalWeight))
