...
public class WeightMenuScreen extends Screen {
    ...
    public WeightMenuScreen(String namespace) {
        super(new TranslatableComponent("screen.ezweight.items_title", namespace));
    ...
    protected void init() {
        ...
        Button backButton = new Button(10, 30, 60, 20,
                new TranslatableComponent("button.ezweight.back"), b ->
                Minecraft.getInstance().setScreen(new WeightNamespaceScreen())
        );
        this.addRenderableWidget(backButton);

        searchBox = new EditBox(this.font, 10, 55, 200, 16, new TranslatableComponent("textbox.ezweight.search"));
        searchBox.setResponder(this::onSearchChanged);
        searchBox.setMaxLength(50);
        this.addRenderableWidget(searchBox);

        clearSearchButton = new Button(215, 55, 20, 16,
                new TextComponent("✕"), b -> searchBox.setValue("")
        );
        this.addRenderableWidget(clearSearchButton);

        Button editButton = new Button(this.width - 140, 30, 120, 20,
                new TranslatableComponent("button.ezweight.edit_selected"), b -> {
        ...
        Button refreshButton = new Button(this.width - 260, 30, 100, 20,
                new TranslatableComponent("button.ezweight.refresh"), b -> {
            refreshItems();
        });
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        String itemCount = new TranslatableComponent("gui.ezweight.item_count", list.children().size() - 1).getString();
        drawString(poseStack, this.font, itemCount, this.width - 200, 10, 0xAAAAAA);
